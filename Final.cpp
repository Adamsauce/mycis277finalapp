...
void rollDice(){
  ...
  int amt,type,mod,roll;
  bool isValid, nullList = false;
  char input, rusure;
  ...
  if(nullList == false){
      //here we ask the suer if they actually ment to roll over one thousand dice, it can happen but not often.
      while(amt > 999){
        if(amt >= 1000000){
      ...
      cout << "\n----------------------------------------" << endl;
      do{
        roll = (rand() % type+1) +mod;
      cout << roll;

      switch(roll){
        default: cout << ", "; break;
        case 20 : critRoll(true); break;
        case 1 : critRoll(false); break;
      }
        amt--;
      }while(amt != 0);
    }
...
int trackInitiative(){
  ...
  int count = 0;
  int player = 1;
  bool isDoneSorting = false, nullList = false;
...
int critRoll(bool crit){
  //if this is called true/false is passed indacating if it was crit hit or crit MISS
  //the proper message is then added
  if(crit == true){
    cout << " CRITICAL HIT! ";
  }else{
    cout << " CRITICAL MISS! ";
